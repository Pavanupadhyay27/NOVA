...
const sprintSteps: SprintStep[] = [
  {
    week: 'Week 01',
    phase: 'Discovery & Scripting',
    badge: 'AUDIENCE & PSYCHOLOGY',
    title: 'Audience Cohort Modeling & 6 Psychological Hooks',
    description: 'We analyze Bhubaneswar buyer psychology across Patia, Saheed Nagar, and Infocity, scripting 6 distinct psychological video angles.',
    deliverables: [
    ...
    specimen: {
      ...
      metrics: [
        { label: 'Hooks Scripted', value: '6 Angles' },
        { label: 'Target Hubs', value: '4 Corridors' },
        { label: 'Audience Pool', value: '180K Reach' },
      ],
  ...
  },
  {
    ...
    description: 'Our creative studio designs, edits, sound-engineers, and color-grades 8 vertical 9:16 video variations with thumb-stopping motion graphics.',
    deliverables: [
    ...
    specimen: {
      ...
      metrics: [
        { label: 'Reel Cuts', value: '8 Videos' },
  ...
  },
  {
    ...
    description: 'We deploy testing budgets using Meta Advantage+ AI, pitting hooks against each other to scientifically identify the top 20% winning creatives.',
    deliverables: [
  ...
  },
  {
    ...
    description: 'With winners validated, we scale ad spend profitably on winning ad sets, expand into lookalike audiences, and rotate fresh creative variants.',
    deliverables: [
  ...
  return (
    <section className={styles.section}>
      <div className="container">
        ...
        <div className={styles.timelineRibbon}>
          {sprintSteps.map((step, idx) => (
            <button
              key={step.week}
              type="button"
              className={`${styles.stepPillBtn} ${activeStepIndex === idx ? styles.stepPillActive : ''}`}
              onClick={() => setActiveStepIndex(idx)}
            >
              <span className={styles.stepNumBadge}>0{idx + 1}</span>
              <div className={styles.stepTextCol}>
                <span className={styles.stepWkTitle}>{step.week}</span>
                <span className={styles.stepPhaseSub}>{step.phase}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        {/* Step Details Stage */}
        <div className={styles.roadmapStageGrid}>
          {/* Left Column: Deliverables */}
          <div className={styles.detailsCol}>
            <div className={styles.badgeRow}>
              <span className={styles.categoryBadge}>{currentStep.badge}</span>
              <span className={styles.slaBadge}>✓ {currentStep.sla}</span>
            </div>

            <h3 className={styles.stepTitle}>{currentStep.title}</h3>
            <p className={styles.stepLead}>{currentStep.description}</p>
            ...
            <div className={styles.deliverablesBox}>
              ...
              <div className={styles.deliverablesGrid}>
                {currentStep.deliverables.map((item) => (
                  <div key={item} className={styles.dItem}>
                    <span className={styles.dCheck}>✓</span>
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            ...
            <div className={styles.actionRow}>
              <Link href="/contact" className={styles.sprintBtn}>
          ...
          {/* Right Column: Specimen Console */}
          <div className={styles.specimenCol}>
            <div className={styles.specimenConsole}>
              <div className={styles.specimenHeader}>
                <span className={styles.specimenDot} />
                <span className={styles.specimenConsoleTitle}>{currentStep.specimen.title}</span>
                <span className={styles.specimenPill}>VERIFIED</span>
              </div>

              <div className={styles.specimenMetricsRow}>
                {currentStep.specimen.metrics.map((m) => (
                  <div key={m.label} className={styles.specimenTile}>
                    <span className={styles.specimenVal}>{m.value}</span>
                    <span className={styles.specimenLbl}>{m.label}</span>
                  </div>
                ))}
              </div>

              <div className={styles.specimenFooter}>
                <span>💡 {currentStep.specimen.note}</span>
              </div>
            </div>
          </div>
        </div>
